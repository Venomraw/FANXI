...
class NationPage(SQLModel, table=True):
    """
    SEO-optimized nation page content generated by HERMES.

    One row per WC2026 qualified nation.  Content is regenerated weekly
    via Groq to keep it fresh for Google crawlers.

    faq_json  : list of {"question": ..., "answer": ...} dicts
    keywords  : list of target keyword strings
    """
    id: Optional[int] = Field(default=None, primary_key=True)
    team: str = Field(unique=True, index=True)             # e.g. "Brazil"
    slug: str = Field(unique=True, index=True)             # e.g. "brazil"
    seo_title: str = Field(default="")
    meta_description: str = Field(default="")              # max 160 chars
    hero_paragraph: str = Field(default="")                # 200-300 words
    history_paragraph: str = Field(default="")             # 150-200 words
