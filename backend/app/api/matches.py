...
from app.models import MatchPrediction, ScoutReport, VisionCache
...
@router.get("/{match_id}/scout-report")
def match_scout_report(match_id: int, session: Session = Depends(get_session)):
    """
    Return the pre-match scout report generated by VISION.
    Returns the cached report if it exists and hasn't expired.
    Does NOT trigger generation — use POST /agents/vision/run for that.
    """
    now = datetime.now(timezone.utc)
    report = session.exec(
        ...
        .order_by(ScoutReport.generated_at.desc())
    ).first()

    if not report:
        raise HTTPException(
            status_code=404,
            detail="No scout report available for this match. Reports are generated by VISION before kickoff.",
        )
...
def match_h2h(match_id: int, session: Session = Depends(get_session)):
    """
    Return head-to-head history for a match generated by VISION.
    Returns cached data only — does NOT trigger Groq generation.
    Use POST /agents/vision/run?run_type=h2h_generation to pre-generate.
    """
    now = datetime.now(timezone.utc)
    # Try direct match_id lookup first
    entry = session.exec(
        select(VisionCache).where(
            VisionCache.cache_type == "h2h",
            VisionCache.match_id == match_id,
        )
    ...
    if not entry:
        raise HTTPException(
            status_code=404,
            detail="No H2H data available for this match. Reports are pre-generated by VISION.",
        )
    ...
    return {
        ...
        "h2h": entry.report_data,
        "generated_at": entry.generated_at.isoformat() if entry.generated_at else None,
    }


@router.get("/{match_id}/post-match-review")
def match_post_review(match_id: int, session: Session = Depends(get_session)):
    """
    Return the post-match community review generated by VISION.
    Returns 404 if the match hasn't been reviewed yet.
    """
    entry = session.exec(
        select(VisionCache).where(
            VisionCache.cache_type == "post_match",
            VisionCache.lookup_key == f"match:{match_id}",
        )
    ).first()
    ...
    if not entry:
        raise HTTPException(
            status_code=404,
            detail="No post-match review available. Reviews are generated by VISION after full-time.",
        )
